...
def plot_epipolar_lines(image1, image2, uncalibrated_1, uncalibrated_2, E, K, plot=True):
    ...
    """ YOUR CODE HERE
    """
    F = np.matmul(np.linalg.inv(K).T, np.matmul(E, np.linalg.inv(K)))
    epipolar_lines_in_1 = np.zeros((3, uncalibrated_1.shape[1]))
    epipolar_lines_in_2 = np.zeros((3, uncalibrated_1.shape[1]))
    for i in range(uncalibrated_1.shape[1]):
        epipolar_lines_in_1[:,i] = np.matmul(uncalibrated_2[:, i], F)
        epipolar_lines_in_2[:,i] = np.matmul( uncalibrated_1[:, i], F.T)
    """ END YOUR CODE
    """
